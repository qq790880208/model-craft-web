    ...
    <div class="content" id="contentfather">
      ...
      >
        <div>
          ...
          >
            <div class="ltc" @mousedown.stop="move(false,true,true, null, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="rtc" @mousedown.stop="move(false,true,true, null, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="lbc" @mousedown.stop="move(false,true,true, null, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="rbc" @mousedown.stop="move(false,true,true, null, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="tc" @mousedown.stop="move(false,false,false,9, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="bc" @mousedown.stop="move(false,false,false,9, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
          </div>
          ...
          >
            ...
            ></div> -->
            <div class="ltc" @mousedown.stop="move(false,false,false,1, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="rtc" @mousedown.stop="move(false,false,false,2, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="lbc" @mousedown.stop="move(false,false,false,3, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="rbc" @mousedown.stop="move(false,false,false,4, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="tc" @mousedown.stop="move(false,false,false,5, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="lc" @mousedown.stop="move(false,false,false,6, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="rc" @mousedown.stop="move(false,false,false,7, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
            <div class="bc" @mousedown.stop="move(false,false,false,8, $event)" v-if="b_i == index"> {{
                index + 1
              }}
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    shotPic() {
      // const node = document.getElementById('contentfather'); // 通过id获取dom
      domtoimage
        .toPng(this.$refs.canvesdiv)
        .then((dataUrl) => {
          ...
          a.download = `标注图片${new Date()}` // 设置图片名称没有设置则为默认
      ...
      document.onmousemove = (e) => {
        ...
        if (istrue === false) {
          if (mtype === 1) {
            ...
            if (left < owidth) {
              this.boxArry[this.b_i].left = oleft + left
            } else {
              this.boxArry[this.b_i].left = oleft + owidth - 1
            }

            if (top < oheight) {
              this.boxArry[this.b_i].top = otop + top
            } else {
              this.boxArry[this.b_i].top = otop + oheight - 1
            }

            if (left < owidth) {
              this.boxArry[this.b_i].width = owidth - left
            } else {
              this.boxArry[this.b_i].width = 1
            }

            if (top < oheight) {
              this.boxArry[this.b_i].height = oheight - top
            } else {
              this.boxArry[this.b_i].height = 1
            }
